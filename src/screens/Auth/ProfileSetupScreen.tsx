...
export const ProfileSetupScreen: React.FC = () => {
		...
		try {
			...
			if (avatarUri) {
				...
				try {
					...
					// Non-blocking: profile can be saved without avatar
				} finally {
					setUploadingAvatar(false);
				}
			}

			// Step 2: Update user profile via user-service
			...
			const { ok, status } = await updateUserProfile(userId, body);

			if (!ok) {
				if (status === 404) {
					// User not found — account may not be provisioned yet.
					// Show a user-friendly message instead of silently ignoring.
					Alert.alert(
						getLocalizedText('notif.error'),
				...
				if (status === 409) {
					Alert.alert(getLocalizedText('notif.error'), "Ce nom d'utilisateur est déjà pris.");
					return;
				}
	...
	const buttonLabel = uploadingAvatar
		? 'Upload avatar...'
				...
				>
					...
					>
						...
						<Button
							...
							loading={loading}
							disabled={
								!firstName.trim() || !lastName.trim() || !username.trim() || loading
							}
							onPress={handleSave}
						/>

...
jest.mock('./src/theme', () => ({
  colors: { text: { light: '#fff' }, primary: { main: '#6200ee' }, ui: { error: '#f00' } },
...
describe('OtpScreen', () => {
  ...
  it('auto-submits and navigates to ConversationsList on successful login', async () => {
    ...
    mockedAuthService.login.mockResolvedValue({ accessToken: 'tok', refreshToken: 'ref' });
    mockedTokenService.decodeAccessToken.mockReturnValue({ sub: 'user1', deviceId: 'dev1' } as any);
  ...
  it('shows error on wrong code', async () => {
    mockedAuthService.confirmVerification.mockResolvedValue({ verified: false });

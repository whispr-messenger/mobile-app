...
describe("reportsAPI", () => {
  ...
  it("retries on 401 after token refresh", async () => {
    const report = { id: "r1" };
    mockFetch
      .mockReturnValueOnce(mockResponse({ error: "Unauthorized" }, 401))
...
describe("rolesAPI", () => {
  it("getMyRole fetches current user role", async () => {
    mockFetch.mockReturnValue(mockResponse({ data: { role: "moderator" } }));
...
describe("auditAPI", () => {
  it("getLogs passes query parameters", async () => {
    mockFetch.mockReturnValue(mockResponse({ data: [], total: 0 }));
